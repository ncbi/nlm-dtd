...
articleauthoring.xsd  [null, default namespace]
...
<article xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         xsi:noNamespaceSchemaLocation="articleauthoring.xsd">

where the xsi:noNamespaceSchemaLocation attribute points to the articleauthoring.xsd module on the system, and the settings appear on the root element of your document (which may or may not be <article>).
...
1. Add the articleauthoring.xsd schema to the list of schemas available. This can be done (though only when a document is open) through the "XML Schema" tab under the "Templates and Add-ins" menu option (under "Tools"). When the schema is selected, Word will want a URI to associate with it (despite the fact that this document type has no default namespace). We have used the following:

Archiving and Interchange XSD
URI: http://namespace.nih.gov/archivearticle.xsd
Alias: NCBI Archive and Interchange schema
...
Alias: NCBI Article Authoring schema

Book XSD
URI: http://namespace.nih.gov/book.xsd
Alias: NCBI Book schema
...
Book Collection XSD
URI: http://namespace.nih.gov/bookcollection.xsd
Alias: NCBI Book Collection schema
